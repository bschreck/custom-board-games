...
def image_prompt_dicts_by_type(type, game_run):
    ...
    return [
        ...
        for prompt_id, prompt_config in config["image_prompts"].items()
...
def gen_component_overview_image(game_run):
    ...
    add_nested_key_to_dict(
        config,
        "images_by_name.components_overview_image",
        {"image_paths": [save_path], "source_images": [c["prompt_id"] for c in component_images], "composite": True},
    )
    save_game_config(game_run, config)
...
def gen_setup_image(game_run):
    ...
    add_nested_key_to_dict(
        config,
        "images_by_name.setup_image",
        {"image_paths": [save_path], "source_images": [component_images[0]["prompt_id"]], "composite": True},
    )
